...
export type ParsedLog = {
  ...
  exercises: { name: string; minutes: number | null; caloriesBurned: number | null }[];
  // ── action="delete" 일 때 ──
  // 그 날짜에서 무엇을 지울지. scope=ALL(그날 전부) / WEIGHT(몸무게만) / MEALS(식사) / EXERCISES(운동)
  // MEALS인데 특정 끼니만이면 mealTypes에 담는다(비어 있으면 그날 식사 전체).
  deleteScope: "ALL" | "WEIGHT" | "MEALS" | "EXERCISES" | null;
...

{
  ...
  "exercises": [
    { "name": "운동명(예:헬스,달리기,산책)", "minutes": 정수 또는 null, "caloriesBurned": 정수 또는 null }
  ],

  // action="delete" 일 때만 채운다 (add면 null/빈배열)
  "deleteScope": "ALL|WEIGHT|MEALS|EXERCISES 또는 null",  // 지울 대상. 통째면 ALL, "몸무게 지워"면 WEIGHT, "점심 지워"면 MEALS, "운동 지워"면 EXERCISES
  "deleteMealTypes": []               // deleteScope=MEALS이고 특정 끼니만 지울 때 그 끼니들. "점심 지워"→["LUNCH"]. 그날 식사 전부면 빈 배열
}

규칙:
- 오직 위 JSON만 출력. 설명·마크다운·코드펜스 금지.
- add일 때 없는 항목을 지어내지 않는다(없으면 빈 배열/ null).
- 음식 칼로리: 한국 1인분 기준 추정(밥 한 공기~300, 삼겹살 1인분~500).
- 운동 소모칼로리: 시간·강도로 추정(빠르게 걷기 ~250/시간, 달리기 ~600/시간, 웨이트 ~350/시간). 시간 모르면 minutes는 null이되 caloriesBurned는 합리적으로 추정.
  ...
  return {
    ...
    exercises: Array.isArray(data.exercises)
      ? data.exercises
          ...
          .map((x) => ({
            name: x.name.trim(),
            minutes: typeof x.minutes === "number" ? Math.round(x.minutes) : null,
            caloriesBurned: typeof x.caloriesBurned === "number" ? Math.round(x.caloriesBurned) : null,
          }))
      : [],
    deleteScope:
      scope === "ALL" || scope === "WEIGHT" || scope === "MEALS" || scope === "EXERCISES" ? scope : null,
